      ...
      if (cardFound.cardType == "Debit") {
        ...
        if (sAccountFound.accountType == "Regular" && amount > 10000)
          return res
            ...
            return res.status(400).json("You Are Not Allowed to Transfer More Than 5,000 , Due to Your Savings Account Type")
        await operationBody.save()
...
router.post("/operations/deposit/:accId", checkToken, validateBody(operationJoi), async (req, res) => {
  try {
    ...
    await operationBody.save()
    if (accountFound.accountType == "Regular" && amount > 100000) return res.status(400).json("You Are Not Allowed to Deposit More Than 100,000 , Due to Your Regular Account Type")
